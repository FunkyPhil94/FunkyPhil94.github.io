// ============================================================
//  CONFIG  –  Change password and guild name here
// ============================================================
const CONFIG = {
  password:  "nato2025",       // ← Change this
  guildName: "NATO [GBR]",
  pages: [
...
// ============================================================
//  AUTH  –  hash-based, works on GitHub Pages without storage
// ============================================================
...
function getAuthHash() {
  return sessionStorage.getItem("nato_auth_hash") || "";
}

async function computeConfigHash() {
  return await hashString(CONFIG.password + "_nato_gbr");
}

async function login(pw) {
  const inputHash  = await hashString(pw + "_nato_gbr");
  const configHash = await computeConfigHash();
  if (inputHash === configHash) {
    sessionStorage.setItem("nato_auth_hash", inputHash);
    return true;
  }
  return false;
}
...
async function isLoggedIn() {
  ...
  const configHash = await computeConfigHash();
  return stored === configHash;
}

function logout() {
  sessionStorage.removeItem("nato_auth_hash");
...
if (!localStorage.getItem("nato_influence")) {
  DB.set("influence", [
    { id: 1, memberId: 1, name: "CommanderX",   week: "2025-W01", influence: 420000, dungeons: 12, events: 5 },
    { id: 2, memberId: 2, name: "SilverArrow",  week: "2025-W01", influence: 310000, dungeons: 9,  events: 4 },
    { id: 3, memberId: 3, name: "IronFrost",    week: "2025-W01", influence: 290000, dungeons: 11, events: 3 },
    { id: 4, memberId: 4, name: "DarkBlade",    week: "2025-W01", influence: 210000, dungeons: 7,  events: 2 },
    { id: 5, memberId: 5, name: "StormBreaker", week: "2025-W01", influence: 180000, dungeons: 6,  events: 4 },
    { id: 6, memberId: 6, name: "LunaWitch",    week: "2025-W01", influence: 160000, dungeons: 8,  events: 3 },
  ]);
}
